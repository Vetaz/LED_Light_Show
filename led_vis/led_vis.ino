...
#define LED_PIN             6
#define TOTAL_NUM_LEDS      300 // Change to 300 for Sam's LED Strip; 60 for Jordi's.
...
uint8_t overallBrightness = 32; // a 0-255 value for how much to scale all leds before writing them out
...
void fillLEDs(uint8_t saturation, uint8_t numLEDs)
{
  ...
  for(int i = 0; i < numLEDs; i++) {
    ...
    if (i % 6 == 0){ // Show filled LEDs every 8 LEDS
//      delay(1000 / UPDATES_PER_SECOND); // For Jordi's Testing
      FastLED.show();
    }
  }
}
...
void unfillLEDs(uint8_t prevNumLEDs, uint8_t numLEDs) { 
  ...
  {
    leds[i] = CRGB(0, 0, 0);
    if (i % 6 == 0) { // Show unfilled LEDs every 8 LEDS
//      delay(1000 / UPDATES_PER_SECOND); // For Jordi's Testing
      FastLED.show();
    }
  }
...
//static int digitalInput[23] = {100,200,300,450,300,100,500,700,800,1023,1023,800,300,200,100,400,500,700,800,500,400,600,300};
//uint8_t index = 0;
